...
import hanto.studentacclab.tournament.HantoPlayer;
...
public class EpsilonHantoMasterTest
{
	...
	@Test
	public void horseJumpsback() throws HantoException
	{
		final PieceLocationPair[] board = new PieceLocationPair[] {
			    plPair(BLUE, BUTTERFLY, 0, 0), plPair(RED, BUTTERFLY, 0, 1),
			    plPair(BLUE, SPARROW, 0, -1), plPair(RED, CRAB, 1, 1),
			    plPair(BLUE, SPARROW, 0, -2), plPair(RED, HORSE, 2, 1),
			    plPair(BLUE, CRAB, 0, -3)
		...
		assertEquals(OK, testGame.makeMove(HORSE, makeCoordinate(2, 1), makeCoordinate(-1, 1)));
		final HantoPiece piece = testGame.getPieceAt(makeCoordinate(-1, 1));
		assertEquals(HORSE, piece.getType());
		assertEquals(RED, piece.getColor());
		assertNull(testGame.getPieceAt(makeCoordinate(2, 1)));
	}
	
	@Test
	public void horseJumpsOnZ() throws HantoException
	{
		final PieceLocationPair[] board = new PieceLocationPair[] {
			    plPair(BLUE, BUTTERFLY, 0, 0), plPair(RED, BUTTERFLY, 1, 0),
			    plPair(BLUE, HORSE, 0, -1), plPair(RED, CRAB, 2, 0), 
			    plPair(BLUE, CRAB, -1, 0), plPair(RED, CRAB, 3, 0), 
			    plPair(BLUE, HORSE, 1, -2), plPair(RED, CRAB, 4, 0)
		};
		testGame.initializeBoard(board);
		testGame.setPlayerMoving(BLUE);
		testGame.setTurnNumber(5);
		assertEquals(OK,
				game.makeMove(HORSE, makeCoordinate(1, -2), makeCoordinate(-2, 1)));
		final HantoPiece piece = game.getPieceAt(makeCoordinate(-2, 1));
		assertEquals(HORSE, piece.getType());
		assertEquals(BLUE, piece.getColor());
		assertNull(game.getPieceAt(makeCoordinate(1, -2)));
	}
	
	@Test
	public void horseJumpsOnZReversed() throws HantoException
	{
		final PieceLocationPair[] board = new PieceLocationPair[] {
			    plPair(BLUE, BUTTERFLY, 0, 0), plPair(RED, BUTTERFLY, 1, 0),
			    plPair(BLUE, HORSE, 0, -1), plPair(RED, CRAB, 2, 0), 
			    plPair(BLUE, CRAB, -1, 0), plPair(RED, CRAB, 3, 0), 
			    plPair(BLUE, HORSE, -2, 1), plPair(RED, CRAB, 4, 0)
		};
		testGame.initializeBoard(board);
		testGame.setPlayerMoving(BLUE);
		testGame.setTurnNumber(5);
		assertEquals(OK,
				game.makeMove(HORSE, makeCoordinate(-2, 1), makeCoordinate(1, -2)));
		final HantoPiece piece = game.getPieceAt(makeCoordinate(1, -2));
		assertEquals(HORSE, piece.getType());
		assertEquals(BLUE, piece.getColor());
		assertNull(game.getPieceAt(makeCoordinate(-2, 1)));
	}
	...
	@Test
	public void printableBoard() throws HantoException{
		final PieceLocationPair[] board = new PieceLocationPair[] {
			    plPair(BLUE, BUTTERFLY, 0, 0), plPair(RED, BUTTERFLY, 1, 0),
			    plPair(BLUE, HORSE, 0, -1), plPair(RED, CRAB, 2, 0), 
			    plPair(BLUE, CRAB, -1, 0), plPair(RED, CRAB, 3, 0), 
	...
	@Test
	public void acceptedResignation() throws HantoException {
		
		final PieceLocationPair[] board = new PieceLocationPair[] {
			    plPair(BLUE, BUTTERFLY, 0, 0), plPair(RED, BUTTERFLY, 1, -1),
			    plPair(RED, HORSE, 0, 1), plPair(RED, HORSE, -1, 0)
		};
		testGame.initializeBoard(board);
		testGame.setPlayerMoving(BLUE);
		testGame.setTurnNumber(3);
		...
		HantoPieceSet pieceSet = ((EpsilonHantoGame ) testGame).getPieceSet();
		
		pieceSet.updatePieceCollection(BLUE, SPARROW);
		pieceSet.updatePieceCollection(BLUE, SPARROW);
		pieceSet.updatePieceCollection(BLUE, CRAB);
		pieceSet.updatePieceCollection(BLUE, CRAB);
		pieceSet.updatePieceCollection(BLUE, CRAB);
		pieceSet.updatePieceCollection(BLUE, CRAB);
		pieceSet.updatePieceCollection(BLUE, CRAB);
		pieceSet.updatePieceCollection(BLUE, CRAB);
		pieceSet.updatePieceCollection(BLUE, HORSE);
		pieceSet.updatePieceCollection(BLUE, HORSE);
		pieceSet.updatePieceCollection(BLUE, HORSE);
		pieceSet.updatePieceCollection(BLUE, HORSE);
		
		
		MoveResult mr = testGame.makeMove(null, null, null);
		assertEquals(RED_WINS, mr);
	}
	
	@Test
	public void testRandomKey() {
		HantoPieceType piece = ((EpsilonHantoGame ) testGame).getPieceSet().getRandomPiece(BLUE);
//		System.out.println(piece.getPrintableName());
	}
	...
	@Test
	public void testPieces() throws HantoException {
		
		HantoPiece p = HantoPieceFactory.getInstance().makeHantoPiece(BLUE, CRAB);
		((BaseHantoPiece) p).isMoveValidAlpha(null, null, 0);
		((BaseHantoPiece) p).isMoveValidBeta(null, null, 0);
		
		p = HantoPieceFactory.getInstance().makeHantoPiece(BLUE, HORSE);
		((BaseHantoPiece) p).isMoveValidAlpha(null, null, 0);
		((BaseHantoPiece) p).isMoveValidBeta(null, null, 0);
		
		p = HantoPieceFactory.getInstance().makeHantoPiece(BLUE, SPARROW);
		((BaseHantoPiece) p).isMoveValidAlpha(null, null, 0);
